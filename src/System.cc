 ...
 */
    System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
        ...
        cv::FileNode node = fsSettings["File.version"];  // 查看配置文件版本，不同版本有不同处理方法
        ...
        node = fsSettings["loopClosing"];  // 是否激活回环，默认是开着的
        bool activeLC = true;
        if (!node.empty()) {
        ...
        bool loadedAtlas = false;  // ORBSLAM3新加的多地图管理功能，这里加载Atlas标识符

        if (mStrLoadAtlasFromFile.empty()) {
            //Load ORB Vocabulary
            cout << endl << "Loading ORB Vocabulary. This could take a while..." << endl;

            mpVocabulary = new ORBVocabulary();  // 建立一个新的ORB字典
            bool bVocLoad = mpVocabulary->loadFromTextFile(strVocFile);  // 读取预训练好的ORB字典并返回成功/失败标志
            if (!bVocLoad) {
            ...
            //Create the Atlas  创建多地图，参数0表示初始化关键帧id为0
        ...
        mpFrameDrawer = new FrameDrawer(mpAtlas);  // 创建用于显示帧和地图的类，由Viewer调用
    ...
    Sophus::SE3f System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point> &vImuMeas,
        ...
        // Check reset
        {
            ...
            if (mbReset) {
                ...
                mbReset = false;
                mbResetActiveMap = false;
            } else if (mbResetActiveMap)  // 如果检测到重置活动地图的标志为true,将重置地图
            {
